...
from kgat_utils.kgat import KGAT
...
if __name__ == "__main__":
            ...
            if args.report != 1:
                ...
                if args.save_flag == -2:
                    user_embed, entity_embed, relation_embed = sess.run(
                        [
                            model.weights["user_embed"],
                            model.weights["entity_embed"],
    ...
    *********************************************************
    ...
    if args.report == 1:
        ...
        f = open(save_path, "w")
        f.write(
            "embed_size=%d, lr=%.4f, regs=%s, loss_type=%s, \n"
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        t1 = time()
        loss, base_loss, kge_loss, reg_loss = 0.0, 0.0, 0.0, 0.0
        ...
        *********************************************************
        ...
        for idx in range(n_batch):
            ...
            (
                ...
                batch_kge_loss,
                batch_reg_loss,
            ) = model.train(sess, feed_dict=feed_dict)
            ...
            if args.use_kge is True:
                ...
                for idx in range(n_A_batch):
                    ...
                    (
                        ...
                        batch_kge_loss,
                        batch_reg_loss,
                    ) = model.train_A(sess, feed_dict=feed_dict)
        ...
        *********************************************************
        ...
        if ret["recall"][0] == cur_best_pre_0 and args.save_flag == 1:
            save_saver.save(
                sess, weights_save_path + "/weights", global_step=epoch
            )
    ...
    f.write(
        "embed_size=%d, lr=%.4f, layer_size=%s, node_dropout=%s, mess_dropout=%s, regs=%s, adj_type=%s, use_att=%s, use_kge=%s, pretrain=%d\n\t%s\n"
        % (
            args.embed_size,
            args.lr,
            args.layer_size,
