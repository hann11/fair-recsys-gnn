...
class KGAT_loader(Data):
    ...
    def _get_relational_adj_list(self):
        ...
        def _np_mat2sp_adj(np_mat, row_pre, col_pre):
            ...
            a_adj = sp.coo_matrix(
                (a_vals, (a_rows, a_cols)), shape=(n_all, n_all)
            )
            b_adj = sp.coo_matrix(
                (b_vals, (b_rows, b_cols)), shape=(n_all, n_all)
            )

            return a_adj, b_adj

        R, R_inv = _np_mat2sp_adj(
            self.train_data, row_pre=0, col_pre=self.n_users
        )
    ...
    def generate_test_feed_dict(
        ...
        feed_dict = {
            ...
            model.mess_dropout: [0.0] * len(eval(self.args.layer_size)),
            model.node_dropout: [0.0] * len(eval(self.args.layer_size)),
        }
