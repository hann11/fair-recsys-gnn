...
class Data(object):
    ...
    def _statistic_ratings(self):
        self.n_users = (
            max(max(self.train_data[:, 0]), max(self.test_data[:, 0])) + 1
        )
        self.n_items = (
            max(max(self.train_data[:, 1]), max(self.test_data[:, 1])) + 1
        )
    ...
    def _print_data_info(self):
        ...
        print(
            "[n_entities, n_relations, n_triples]=[%d, %d, %d]"
            % (self.n_entities, self.n_relations, self.n_triples)
        )
        print(
            "[batch_size, batch_size_kg]=[%d, %d]"
            % (self.batch_size, self.batch_size_kg)
        )
        ...
        def sample_neg_items_for_u(u, num):
            ...
            while True:
                if len(neg_items) == num:
                    break
                neg_i_id = np.random.randint(low=0, high=self.n_items, size=1)[
                    0
                ]

                if (
                    neg_i_id not in self.train_user_dict[u]
                    and neg_i_id not in neg_items
                ):
        ...
        try:
            ...
            for idx, line in enumerate(lines):
                if idx % 2 == 0:
                    ...
                    split_uids.append(
                        [int(uid) for uid in line.strip().split(" ")]
                    )
