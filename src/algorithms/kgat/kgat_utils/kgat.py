...
class KGAT(object):
    ...
    def _build_weights(self):
        ...
        if self.pretrain_data is None:
            all_weights["user_embed"] = tf.Variable(
                initializer([self.n_users, self.emb_dim]), name="user_embed"
            )
            all_weights["entity_embed"] = tf.Variable(
                initializer([self.n_entities, self.emb_dim]),
                name="entity_embed",
            )
            ...
            other_embed = initializer(
                [self.n_entities - self.n_items, self.emb_dim]
            )

            all_weights["entity_embed"] = tf.Variable(
                initial_value=tf.concat([item_embed, other_embed], 0),
                trainable=True,
                name="entity_embed",
                dtype=tf.float32,
            )
        ...
        all_weights["relation_embed"] = tf.Variable(
            initializer([self.n_relations, self.kge_dim]), name="relation_embed"
        )
        all_weights["trans_W"] = tf.Variable(
            initializer([self.n_relations, self.emb_dim, self.kge_dim])
        )
    ...
    def _build_model_phase_I(self):
        if self.alg_type in ["bi", "kgat"]:
            (
                self.ua_embeddings,
                self.ea_embeddings,
            ) = self._create_bi_interaction_embed()
            ...
            (
                self.ua_embeddings,
                self.ea_embeddings,
            ) = self._create_graphsage_embed()
        else:
            print(
                "please check the the alg_type argument, which should be bi, kgat, gcn, or graphsage."
            )
            raise NotImplementedError

        self.u_e = tf.nn.embedding_lookup(self.ua_embeddings, self.users)
        self.pos_i_e = tf.nn.embedding_lookup(
            self.ea_embeddings, self.pos_items
        )
        self.neg_i_e = tf.nn.embedding_lookup(
            self.ea_embeddings, self.neg_items
        )

        self.batch_predictions = tf.matmul(
            self.u_e, self.pos_i_e, transpose_a=False, transpose_b=True
        )

    def _build_model_phase_II(self):
        self.h_e, self.r_e, self.pos_t_e, self.neg_t_e = self._get_kg_inference(
            self.h, self.r, self.pos_t, self.neg_t
        )
        self.A_kg_score = self._generate_transE_score(
            h=self.h, t=self.pos_t, r=self.r
        )
    ...
    def _build_loss_phase_I(self):
        ...
        regularizer = (
            tf.nn.l2_loss(self.u_e)
            + tf.nn.l2_loss(self.pos_i_e)
            + tf.nn.l2_loss(self.neg_i_e)
        )
    ...
    def _build_loss_phase_II(self):
        def _get_kg_score(h_e, r_e, t_e):
            kg_score = tf.reduce_sum(
                tf.square((h_e + r_e - t_e)), 1, keepdims=True
            )
            return kg_score
        ...
        kg_reg_loss = (
            tf.nn.l2_loss(self.h_e)
            + tf.nn.l2_loss(self.r_e)
            + tf.nn.l2_loss(self.pos_t_e)
            + tf.nn.l2_loss(self.neg_t_e)
        )
    ...
    def _create_graphsage_embed(self):
        ...
        for k in range(self.n_layers):
            ...
            for f in range(self.n_fold):
                temp_embed.append(
                    tf.sparse_tensor_dense_matmul(A_fold_hat[f], pre_embeddings)
                )
            embeddings = tf.concat(temp_embed, 0)
            ...
            pre_embeddings = tf.nn.dropout(
                pre_embeddings, 1 - self.mess_dropout[k]
            )
    ...
    def train(self, sess, feed_dict):
        return sess.run(
            [self.opt, self.loss, self.base_loss, self.kge_loss, self.reg_loss],
            feed_dict,
        )
