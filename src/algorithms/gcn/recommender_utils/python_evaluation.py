...
def merge_ranking_true_pred(
    ...
    df_hit_count = pd.merge(
        df_hit.groupby(col_user, as_index=False)[col_user].agg(
            {"hit": "count"}
        ),
...
def ndcg_at_k(
    ...
    **kwargs,
    ...
    df_user = df_dcg.groupby(col_user, as_index=False, sort=False).agg(
        {"dcg": "sum"}
    )
...
def map_at_k(
    ...
    **kwargs,
    ...
    df_hit_sorted = (
        df_hit_sorted.groupby(col_user).agg({"rr": "sum"}).reset_index()
    )
...
def get_top_k_items(
    ...
    if k is None:
        ...
        top_k_items = (
            dataframe.sort_values(
                [col_user, col_rating], ascending=[True, False]
            )
    ...
    # Add ranks
    top_k_items["rank"] = (
        top_k_items.groupby(col_user, sort=False).cumcount() + 1
    )
    return top_k_items
...
def _get_rating_column(relevancy_method: str, **kwargs) -> str:
    ...
    if relevancy_method != "top_k":
        if "col_rating" not in kwargs:
            raise ValueError(
                "Expected an argument `col_rating` but wasn't found."
            )
    ...
    * check column names in item_feature_df
    ...
    def check_column_dtypes_diversity_serendipity_wrapper(
        ...
        **kwargs,
        ...
        if not has_same_base_dtype(
            train_df, reco_df, columns=[col_user, col_item]
        ):
            raise ValueError(
                "Columns in provided DataFrames are not the same datatype"
            )
        ...
        if item_sim_measure == "item_feature_vector":
            ...
            if item_feature_df is not None:
                if not has_columns(item_feature_df, required_columns):
                    raise ValueError(
                        "Missing columns in item_feature_df DataFrame"
                    )
            else:
                raise Exception(
...
def _get_pairwise_items(
    ...
    df_pairwise_items = df_user_i1_i2[
        (df_user_i1_i2["i1"] <= df_user_i1_i2["i2"])
    ][[col_user, "i1", "i2"]].reset_index(drop=True)
...
def _get_item_feature_similarity(
    ...
    df_cosine_similarity = df_item_feature_pair[
        ["i1", "i2", col_sim]
    ].sort_values(["i1", "i2"])
...
def novelty(
    ...
    df_item_novelty = historical_item_novelty(
        train_df, reco_df, col_user, col_item
    )
