...
from recommender_utils.python_evaluation import (
    map_at_k,
    ndcg_at_k,
    precision_at_k,
    recall_at_k,
)
from recommender_utils.python_utils import get_top_k_scored_items
...
class LightGCN(object):
    ...
    def _create_lightgcn_embed(self):
        ...
        for k in range(0, self.n_layers):
            ego_embeddings = tf.sparse.sparse_dense_matmul(
                A_hat, ego_embeddings
            )
            all_embeddings += [ego_embeddings]
    ...
    def _create_bpr_loss(self, users, pos_items, neg_items):
        ...
        pos_scores = tf.reduce_sum(
            input_tensor=tf.multiply(users, pos_items), axis=1
        )
        neg_scores = tf.reduce_sum(
            input_tensor=tf.multiply(users, neg_items), axis=1
        )
    ...
    def fit(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for idx in range(n_batch):
                users, pos_items, neg_items = self.data.train_loader(
                    self.batch_size
                )
            ...
            if self.save_model and epoch % self.save_epoch == 0:
                save_path_str = os.path.join(
                    self.model_dir, "epoch_" + str(epoch)
                )
                ...
                print(
                    "Save model to path {0}".format(
                        os.path.abspath(save_path_str)
                    )
                )
    ...
    def run_eval(self):
        ...
        for metric in self.metrics:
            if metric == "map":
                ...
                ret.append(
                    precision_at_k(self.data.test, topk_scores, k=self.top_k)
                )
            elif metric == "recall":
                ret.append(
                    recall_at_k(self.data.test, topk_scores, k=self.top_k)
                )
        return ret
    ...
    def score(self, user_ids, remove_seen=True):
        ...
        for u_batch_id in range(n_user_batchs):
            ...
            rate_batch = self.sess.run(
                self.batch_ratings,
                {self.users: user_batch, self.pos_items: item_batch},
            )
        ...
        df = pd.DataFrame(
            {
                data.col_user: np.repeat(
                    test[data.col_user].drop_duplicates().values,
                    top_items.shape[1],
                ),
